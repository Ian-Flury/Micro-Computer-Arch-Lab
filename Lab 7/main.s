...
Key_ASCII	SPACE	4	; ASCII value of the pressed key (the output of keypad)
...
loop
	...
	LDR 	R0, =RFlag
	LDR 	R1, [R0] 
	CMP 	R1, #1 		;check if button pressed in col 0
	BEQ 	skip
	
	BL 		Scan_Col_1
	LDR 	R0, =RFlag
	LDR 	R1, [R0]
	CMP 	R1, #1 		;check if button pressed in col 1
	BEQ 	skip
	
	BL 		Scan_Col_2
	LDR 	R0, =RFlag
	LDR 	R1, [R0]
	CMP 	R1, #1 		;check if button pressed in col 2
	BEQ 	skip
	
	BL 		Scan_Col_3
	LDR 	R0, =RFlag
	LDR 	R1, [R0]
	CMP 	R1, #1 		;check if button pressed in col 3
	BNE 	loop
...
Read_PortD	
	...
	PUSH	{R0-R2,LR}			
	;******************************************
	; TO DO: Reset the RFlag.
	;******************************************
	LDR 	R0, =RFlag
	MOV 	R1, #0
	STR	 	R1, [R0]
	;******************************************
	; To DO: Read PortD into R1, and store R1
	...
	STR 	R1, [R0]
	;******************************************
	; TO DO: Now complete the rest of this 
	; subroutine as specified in the lab guide.
	;******************************************
	CMP 	R1,#0x00 	;the button is pressed if R1 == 1
	BEQ 	skip_end 	;else, its not, skip to the end
	MOV 	R0, #90
	BL 		Delay1ms 	;delay for 90ms to debounce the button press
	LDR 	R2, =GPIO_PORTD_DATA_R
	LDR		R0, [R2]
 	AND 	R0, #0x0F 	;filter the button press data from PORTD
	CMP 	R1, R0 		;check if the value is the same as before the debounce delay
	BNE	 	skip_end 	;if not, the button was not pressed, skip to end.
	
	LDR 	R0, =RFlag
	MOV 	R1, #1
	STR	 	R1, [R0] 	;update the RFlag
