...
; Date Modified: 03/1/2022 - Friendly codes are used as well as modified subroutines
...
__main
	...
	BL		Init_Ports ;
	BL		Init_LCD
	MOV		R1, 0x00 ; TO DO: Complete this instruction by setting the position of the first char in line 1
	BL		Set_Position    ; Sets the position for the first line
	LDR		R0, =Msg_hd1	; R0 <- address of the message header 1
	BL		Display_Msg	;
	MOV		R1, 0x43 ; TO DO: Complete this instruction by setting the position of the first char in line 2
	BL		Set_Position	;
...
Init_Ports
	...
	STR		R3, [R2]
	; TO DO: The previous four instructions enable digital data on PA5-PA2 in a friendly
	; manner. Use a similar approach to enable digital data on PC6 and PE0 (8 instructions)
...
Init_LCD
	...
	MOV		R1,	#0x0C		; R1 <- byte #5 of code: $28
				;  db5 = 1, db4 = 0 (DL = 0 - 4 bits), 
				;  db3 = 1 (N = 1 - 2 lines),
				;  db2 = 0 (F = 0 - 5x7 dots).
	BL		SplitNum	;
	...
	MOV		R1,	#0x01		; R1 <- byte #5 of code: $28
				;  db5 = 1, db4 = 0 (DL = 0 - 4 bits), 
				;  db3 = 1 (N = 1 - 2 lines),
				;  db2 = 0 (F = 0 - 5x7 dots).
	BL		SplitNum	;
	...
	; TO DO: Refer to your pre-lab table of command bytes and the instructions
	; above for sending byte 5 to send byte 6.
	MOV		R1,	#0x06		; R1 <- byte #5 of code: $28
				;  db5 = 1, db4 = 0 (DL = 0 - 4 bits), 
				;  db3 = 1 (N = 1 - 2 lines),
				;  db2 = 0 (F = 0 - 5x7 dots).
	BL		SplitNum	;
